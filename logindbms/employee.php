...
<body>
    <div class="nav">
        ...
        <div class="d-flex flex-row">
            <div class="right-links">
                <div class="d-flex flex-row">
                    <div>
                        <a href="ongoing.php"><button class="btn">On going</button>
                    </div>
                    <div>
            ...
            <div class="container">
                <div class="card-deck posts_box">
                    ...
                    <div class="card col-sm-5"
                        onclick="showPostDetails('<?php echo $post_id; ?>', '<?php echo $post_title; ?>', '<?php echo $post_dur; ?>', '<?php echo $post_price; ?>', '<?php echo $post_count; ?>', '<?php echo $post_timestamp; ?>')">
                        <h4>Pending Posts</h4>
        ...
        <div id="section2" style="display:none;">
            <div class="container">
                <div class="post-details">
                    ...
                    <button class="btn" onclick="takePost()">Take</button>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        function showPostDetails(postId, postTitle, postDur, postPrice, postCount, postTimestamp) {
            document.getElementById('postTitle').innerText = postTitle;
            document.getElementById('postDuration').innerText = 'Duration: ' + postDur + ' hr';
            document.getElementById('postPrice').innerText = 'Price: Rs. ' + postPrice + ' /-';
            document.getElementById('postCount').innerText = 'Count: ' + postCount;
            document.getElementById('postTimestamp').innerText = 'Last updated: ' + postTimestamp;
            document.getElementById('section1').style.display = 'none';
            document.getElementById('section2').style.display = 'block';

            // Store the post ID in a hidden input for later use
            const hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.id = 'postId';
            hiddenInput.value = postId;
            document.querySelector('.post-details').appendChild(hiddenInput);
        }

        function takePost() {
            const postId = document.getElementById('postId').value;

            // Send an AJAX request to update the post status and move to ongoing posts
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'take_post.php', true);
